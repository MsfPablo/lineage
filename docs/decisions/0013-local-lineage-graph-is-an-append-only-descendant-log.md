...
A receiver (or the maintainer) can already ask "what package did this project's enabled state come from, and when" for anything enabled after this change, without needing issue #7's snapshot store or any session concept to exist first. The record format doesn't need to change shape when those land — it just stops leaving `snapshot_id`/`session_id` empty. The cost: enabling a package now does one more small disk write, and until the deferred wiring lands, `import`/`pull`/`run` don't yet produce descendant records, so the graph is incomplete relative to the issue's full "why" until that follow-up work happens.
...
- Once issue #7 (content-addressed snapshot store) exists, populate `ParentRef.SnapshotID` from it instead of leaving it empty.
